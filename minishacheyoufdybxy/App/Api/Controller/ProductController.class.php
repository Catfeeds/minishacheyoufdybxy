...
class ProductController extends PublicController {
	...
	public function get_buff(){
   	...
   	public function lists(){
 		...
 		$brand_id=intval($_POST['brand_id']);//获得品牌id 这里的id是brand表里的cid
 		// $id=44;
	...
	public function myshop_prolists(){
		...
		$shopid=M("user")->where("id=$uid")->getField("shop_id");
 		...
 		$limit=($page*40)-40;
 		$where="1=1 AND pro_type=1 AND del=0 AND is_down=0 AND shop_id=".$shopid;

 		$product=M('product')->where($where)->order("addtime desc,id desc")->limit($limit,40)->select();
