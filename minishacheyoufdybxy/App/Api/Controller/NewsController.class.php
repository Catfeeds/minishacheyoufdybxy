...
class NewsController extends PublicController {
    ...
    //*****************************
    public function cat(){
        $json=M('category')->where('tid=23')->field('id,name')->order('sort desc,bz_2 desc')->select();
    	foreach ($json as $k => $v) {
    ...
    public function lists(){
        ...
        $newscat=M('news')->where($where)->field('id,name,photo,addtime,from')->order('addtime desc')->limit(20)->select();
        $json=array();
        foreach ($newscat as $k => $v) {
            $newscat[$k]['photo']="http://".$_SERVER['SERVER_NAME'].__DATA__.'/'.$v['photo'];
            $newscat[$k]['name']=html_entity_decode($v['name'], ENT_QUOTES, "utf-8");
            $newscat[$k]['addtime']=date('Y-m-d',$v['addtime']);
        }
        //json加密输出
        //dump($json);
        echo json_encode(array('status'=>1,'newslist'=>$newscat));
        exit();
    }
    /**
     * [getlist 加载更多资讯]
    ...
    public function getlist(){
        ...
        $newscat=M('news')->where($where)->field('id,name,photo,addtime,from')->order('addtime desc')->limit($limit.',20')->select();
        $json=array();
        foreach ($newscat as $k => $v) {
            $newscat[$k]['photo']="http://".$_SERVER['SERVER_NAME'].__DATA__.'/'.$v['photo'];
            $newscat[$k]['name']=html_entity_decode($v['name'], ENT_QUOTES, "utf-8");
    ...
    public function detail(){
        ...
        $json=array();
        $json['status']=1;
        $json['err']['id']=$detail['id'];
        $json['err']['name']=html_entity_decode($detail['name'], ENT_QUOTES, "utf-8");
        $json['err']['from']=html_entity_decode($detail['from'], ENT_QUOTES, "utf-8");
        $json['err']['addtime']=date('Y-m-d',$detail['addtime']);
        $json['err']['renqi']=$detail['renqi'];
        $content = str_replace(C('content.dir'), __DATAURL__ , $detail['content']);
        $json['err']['content']=html_entity_decode($content, ENT_QUOTES, "utf-8");
        //json加密输出
        //dump($json);
