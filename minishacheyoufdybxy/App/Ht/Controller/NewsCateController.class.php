...
class NewsCateController extends PublicController{
	...
	*/
	public function index(){
		...
		//获取分类表里所有新闻分类
		$cate_list = $this->category->where('tid=10')->order('sort desc,id desc')->select();
	...
	*/
	public function save(){
		...
		//判断是否已经存在该栏目
		if (!intval($_POST['cid'])) {
			$check_id = $this->category->where('tid=10 AND name="'.trim($_POST['name']).'"')->getField('id');
			if (is_int($check_id)) {
				$this->error('栏目已存在.');
			}
		}
